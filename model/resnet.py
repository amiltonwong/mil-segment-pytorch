...
def make_layer(block, in_channels, channels, num_blocks, stride=1, dilation=1):
    ...
    for stride in strides:
        # block is nn.Module, blocks is list
        blocks.append(block(in_channels=in_channels, channels=channels, stride=stride, dilation=dilation))
        in_channels = block.expansion*channels # BasicBlock expansion=1
...
class ResNet_BasicBlock_OS8(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            resnet = models.resnet18() # use torchvision model, but not use imagenet pre-trained weights
            # load pretrained model:
            resnet.load_state_dict(torch.load("/root/deeplabv3/pretrained_models/resnet/resnet18-5c106cde.pth"))
            # remove fully connected layer, avg pool, layer4 and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-4]) # use * to remove list type
