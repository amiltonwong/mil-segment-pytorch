...
class DatasetTrain(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_img = cv2.imread(label_img_path, -1) # (shape: (1024, 2048)) # use flag=-1 to read as greyscale image for label
        ...
        flip = np.random.randint(low=0, high=2) # get 0 or 1
        if flip == 1: # half chance to get flip
            img = cv2.flip(img, 1) # laterally inverse
            label_img = cv2.flip(label_img, 1)

        ########################################################################
        # randomly scale the img and the label:
        ########################################################################
        scale = np.random.uniform(low=0.7, high=2.0) # get scale value
        new_img_h = int(scale*self.new_img_h) # at most scale to 1024x2048
        new_img_w = int(scale*self.new_img_w) # at least scale to 358x716
        ...
        img = img/255.0
        img = img - np.array([0.485, 0.456, 0.406]) # ImageNet mean value: [0.485, 0.456, 0.406] & std value: [0.229, 0.224, 0.225]
        img = img/np.array([0.229, 0.224, 0.225]) # (shape: (256, 256, 3))
        img = np.transpose(img, (2, 0, 1)) # (shape: (3, 256, 256)) # transpose to format of pytorch: channel first
        img = img.astype(np.float32) # convert to np.float32 type
...
class DatasetSeq(torch.utils.data.Dataset):
    def __init__(self, cityscapes_data_path, cityscapes_meta_path, sequence):
        self.img_dir = "/data/cityscapes_dataset/leftImg8bit_demoVideo" + "/leftImg8bit/demoVideo/stuttgart_" + sequence + "/"

        self.img_h = 1024
        self.img_w = 2048
        # download sample (/2) into 512, 1024
        self.new_img_h = 512
        self.new_img_w = 1024

        self.examples = []

        file_names = os.listdir(self.img_dir) # img_dir: /data/cityscapes_dataset/leftImg8bit_demoVideo/leftImg8bit/demoVideo/stuttgart_00
        for file_name in file_names:
            img_id = file_name.split("_leftImg8bit.png")[0] # name: stuttgart_00_000000_000001_leftImg8bit.png
                                                            # id:   stuttgart_00_000000_000001
            img_path = self.img_dir + file_name

            example = {} # create an empty dictionary
            example["img_path"] = img_path
            example["img_id"] = img_id
            self.examples.append(example) # examples is list, example is dictionary
